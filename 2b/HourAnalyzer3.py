...
data['connectionDate'] = data['connectionTime'].dt.date
...
# Calculate total possible occupancy minutes per hour across all working days
# Get the number of weekdays in the dataset
weekdays = pd.date_range(
    start=data['connectionTime'].dt.date.min(),
    end=data['disconnectTime'].dt.date.max(),
    freq='B'  # 'B' generates only business days (Mon-Fri)
)
num_weekdays = len(weekdays)

hourly_occupancy['total_possible_minutes'] = num_weekdays * total_stations * 60  # 60 minutes per hour
...
plt.title('Average Utilization Rate by Hour of Day (Weekdays Only)')
...
plt.title('Average Session Duration by Hour of Day (Weekdays Only)')
plt.xlabel('Hour of Day')
