...
# symLog auf Features anwenden
def sym_log_transform(x):
    return np.sign(x) * np.log1p(np.abs(x))

log_features = features.applymap(sym_log_transform)

# Features skalieren
scaler = StandardScaler()
scaled_log_features = scaler.fit_transform(log_features)

# DBSCAN Clustering durchführen
dbscan = DBSCAN(eps=0.5, min_samples=5)  # eps und min_samples ggf. anpassen
charging_data["cluster"] = dbscan.fit_predict(scaled_log_features)

# Silhouette-Score berechnen (nur wenn mehr als 1 Cluster vorhanden)
if len(set(charging_data["cluster"])) > 1:
    silhouette_avg = silhouette_score(scaled_log_features, charging_data["cluster"])
    print(f"Silhouette Score: {silhouette_avg}")
else:
    print("Silhouette Score konnte nicht berechnet werden (nur ein Cluster gefunden).")
...
output_path = "3/clustered_charging_data_dbscan.csv"
charging_data_clustered.to_csv(output_path, index=False)
print(f"Clustered data with DBSCAN saved to {output_path}")

import matplotlib.pyplot as plt

plt.scatter(scaled_log_features[:, 0], scaled_log_features[:, 1], c=charging_data["cluster"], cmap='viridis', s=5)
